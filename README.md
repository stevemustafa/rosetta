...
```bash
rosetta$ sam build --use-container
```
...
```bash
rosetta$ sam logs -n StockCheckerFunction --stack-name rosetta --tail
```
...
```bash
rosetta$ pip install pytest pytest-mock --user
rosetta$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name rosetta
```
